...
struct FunctionSymbolTableEntry {
    ...
    FunctionSymbolTableEntry(const std::string &functionPrototype, const std::string &returnType, int numberOfArgs,
                             const std::vector<std::string> &argTypes, const yy::location &location)
            : function_prototype(functionPrototype), return_type(returnType), number_of_args(numberOfArgs),
              arg_types(argTypes), location(location) {}

    friend std::ostream &operator<<(std::ostream &out, FunctionSymbolTableEntry &entry) {
...
class ScopeStack {
    std::vector<std::variant<std::unique_ptr<SymbolTableEntry>, std::unique_ptr<FunctionSymbolTableEntry>>> entries;
...
public:
    ...
    SymbolTableEntry *lookup(Identifier anIdentifier) {
        ...
        error(anIdentifier.location, " identifier '", anIdentifier.name, "' not defined");
        return nullptr;
    }
